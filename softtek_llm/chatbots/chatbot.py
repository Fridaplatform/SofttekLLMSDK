...
class Chatbot:
    ...
    def _call_model(self, logging_kwargs: Dict | None = None) -> Response:
        """
        This method is used to call the model and returns a Response object.

        Args:
            `logging_kwargs` (Dict, optional): additional keyword arguments to be passed to the logging function. Defaults to None.
    ...
    def chat(
        ...
        if not self.cache:
            last_message = self._call_model(logging_kwargs=logging_kwargs)
        else:
